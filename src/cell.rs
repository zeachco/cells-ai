...
impl Cell {
    ...
    pub fn render(&self, camera_x: f32, camera_y: f32) {
        ...
        // Draw a line showing the direction the cell is facing
    ...
    pub fn forward_with_angle(&mut self, angle: f32) {
        if self.energy >= 1.0 {
            let speed = 2.0;
            self.velocity_x += angle.cos() * speed;
            self.velocity_y += angle.sin() * speed;
            self.energy -= 1.0;
        }
    }

    pub fn random_forward(&mut self) {
        if rand::gen_range(0.0, 1.0) > 0.5 {
            let random_angle = rand::gen_range(0.0, std::f32::consts::TAU);
            self.forward_with_angle(random_angle);
        }
    }
